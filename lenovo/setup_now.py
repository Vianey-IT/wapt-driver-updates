...
TASK_NAME  = 'WAPT-DriverUpdate-Lenovo-Now'
SCRIPT_DST = r'C:\Scripts\wapt\run_driver_updates_lenovo_now.ps1'
...
def install():
    logger.info('=== Lancement mise a jour drivers + BIOS Lenovo (LSUClient) ===')
    logger.info('CWD : %s', os.getcwd())

    script_src      = find_file('run_driver_updates_lenovo_now.ps1')
    create_task_src = find_file('create_task_lenovo_now.ps1')

    if not script_src:
        raise FileNotFoundError('run_driver_updates_lenovo_now.ps1 introuvable dans le paquet')
    if not create_task_src:
        raise FileNotFoundError('create_task_lenovo_now.ps1 introuvable dans le paquet')

    os.makedirs(os.path.dirname(SCRIPT_DST), exist_ok=True)
    shutil.copy2(script_src, SCRIPT_DST)
    logger.info('Script copie vers : %s', SCRIPT_DST)

    # Créer et déclencher la tâche temporaire
    proc = subprocess.run([
        'powershell.exe',
        '-NonInteractive',
