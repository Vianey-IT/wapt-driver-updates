...
TASK_NAME  = 'WAPT-DriverUpdate-Lenovo'
SCRIPT_DST = r'C:\Scripts\wapt\run_driver_updates_lenovo.ps1'
...
def install():
    logger.info('=== Installation tache planifiee Lenovo ===')
    logger.info('CWD : %s', os.getcwd())

    script_src      = find_file('run_driver_updates_lenovo.ps1')
    create_task_src = find_file('create_task_lenovo.ps1')

    if not script_src:
        raise FileNotFoundError('run_driver_updates_lenovo.ps1 introuvable dans le paquet')
    if not create_task_src:
        raise FileNotFoundError('create_task_lenovo.ps1 introuvable dans le paquet')
...
def uninstall():
    logger.info('=== Suppression tache planifiee Lenovo ===')
    subprocess.call([
...
def audit():
    try:
        ...
        if exit_code in (0, 1, 3010):
            print('OK : Tache presente - derniere execution {} - code {}'.format(last_run, exit_code))
            return 'OK'
        else:
