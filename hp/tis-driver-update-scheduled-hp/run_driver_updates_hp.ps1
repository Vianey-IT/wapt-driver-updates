...
# Tache planifiee WAPT-DriverUpdate-HP - drivers sans BIOS ni Firmware via HPCMSL - compte SYSTEM

$LogDir = "C:\Logs\wapt-drivers"
if (-not (Test-Path $LogDir)) { New-Item -ItemType Directory -Path $LogDir | Out-Null }
$LogFile = Join-Path $LogDir ("hp_update_" + (Get-Date -Format "yyyy-MM-dd_HH-mm") + ".log")
...
try {
    Write-Log "--- Demarrage HPCMSL (scheduled - drivers sans BIOS ni Firmware) ---"
    ...
    Write-Log "Recherche des mises a jour disponibles (drivers sans BIOS ni Firmware)..."
    $softpaqs = Get-HPSoftpaqList -Category Driver,Dock -Characteristic SSM
    ...
    if ($success -eq 0 -and $errors -eq 0) {
        ...
        Write-Registry -Status "OK - MAJ installees" -Detail "$success MAJ drivers installees via HPCMSL." -ExitCode 1
    } else {
