...
TASK_NAME  = 'WAPT-DriverUpdate-Dell'
SCRIPT_DST = r'C:\Scripts\wapt\run_driver_updates_dell.ps1'
...
def install():
    logger.info('=== Installation tache planifiee Dell ===')
    logger.info('CWD : %s', os.getcwd())

    script_src      = find_file('run_driver_updates_dell.ps1')
    create_task_src = find_file('create_task_dell.ps1')

    if not script_src:
        raise FileNotFoundError('run_driver_updates_dell.ps1 introuvable dans le paquet')
    if not create_task_src:
        raise FileNotFoundError('create_task_dell.ps1 introuvable dans le paquet')
...
def uninstall():
    logger.info('=== Suppression tache planifiee Dell ===')
    subprocess.call([
